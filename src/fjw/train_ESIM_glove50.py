...
#                                         bidirectional=True)
        ...
        self._composition = Seq2SeqEncoder(nn.LSTM,
                                           self.hidden_size*2,
                                           self.hidden_size,
                                           bidirectional=True) #这个地方的hidden_size也许可以乘以2
        ...
        self.decoder_gender=nn.Sequential(nn.Dropout(p=self.dropout),
                                             nn.Linear(4*2*self.hidden_size+55,
                                                       self.hidden_size),
                                             nn.Tanh(),
...
#     model=ESIM_Single(config,0,device=device)
    output_dir="../../model/fjw/model_ESIM_glove50_"+str(fold+1)+"/"
#     output_dir="./model_ESIM_Single_wv50_"+str(fold+1)+"/"
#     output_dir="./model_ESIM_glove50_"+str(fold+1)+"/"
#     output_dir="./model_ESIM_wv50_0.003_shuffle_length100_reverse_"+str(fold+1)+"/"
    card_list=[0,2,3]
