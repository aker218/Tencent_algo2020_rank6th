...
tfidf_sequence_info=pk.load(open("../../var/fjw/full_tfidf.pickle","rb"))
textrank_sequence_info=pk.load(open("../../var/fjw/textrank/full_textrank.pickle","rb"))
...
#                                         bidirectional=True)
        ...
        self._composition = Seq2SeqEncoder(nn.LSTM,
                                           ...
                                           self.hidden_size,
                                           bidirectional=True)
        self.ln_create=LayerNorm(200)
        self.ln_ad=LayerNorm(200)
        self.ln_product_id=LayerNorm(100)
        self.ln_product_cat=LayerNorm(100)
        self.ln_advertiser=LayerNorm(200)
        self.ln_industry=LayerNorm(100)
#         self.ln_create_g=LayerNorm(50)
#         self.ln_ad_g=LayerNorm(50)
#         self.ln_product_id_g=LayerNorm(50)
#         self.ln_product_cat_g=LayerNorm(50)
#         self.ln_advertiser_g=LayerNorm(50)
#         self.ln_industry_g=LayerNorm(50)
        
        
        self.ln_advertiser_id_industry=LayerNorm(100)
        self.ln_product_cat_industry=LayerNorm(100)
        self.ln_product_cat_advertiser=LayerNorm(100)
        self.ln_product_id_advertiser=LayerNorm(100)
        self.ln_product_id_industry=LayerNorm(100)
        self.ln_product_id_product_cat=LayerNorm(100)

#         self.ln_advertiser_id_industry_g=LayerNorm(50)
#         self.ln_product_cat_industry_g=LayerNorm(50)
#         self.ln_product_cat_advertiser_g=LayerNorm(50)
#         self.ln_product_id_advertiser_g=LayerNorm(50)
#         self.ln_product_id_industry_g=LayerNorm(50)
#         self.ln_product_id_product_cat_g=LayerNorm(50)
...
#                            self.ln_product_id(product_id_embeddings[:,:,:]),\
        ...
        premises=torch.cat([self.ln_create(create_embeddings[:,:,:]),                            self.ln_ad(ad_embeddings[:,:,:]),                            norm_target,norm_time,                            norm_click_time],dim=-1)
...
class Ad_Transformer(nn.Module):
    def __init__(self,config,device):
        super(Ad_Transformer,self).__init__()
        self.config=config
        self.Encoder=BertEncoder(config=config)
        self.P=PositionalEncoding(config)
        self.rnns=nn.ModuleList()
        self.device=device
#  nn.Sequential(GCNN_block(hidden_size,hidden_size,3,padding=1,dilation=1))
        for i in range(1):
            hidden_size=config.hidden_size
            self.rnns.append(nn.GRU(input_size=config.hidden_size,hidden_size=config.hidden_size,num_layers=1,                        bidirectional=True,batch_first=True))
        ...
        self.ln_tfidf_sequence=LayerNorm(6)
        self.ln_tr_sequence=LayerNorm(6)
        self.ln_graph_sequence=LayerNorm(42)
        self.ln_one_hot_target=LayerNorm(72)
        self.ln_time=LayerNorm(32)
        self.ln_click_times=LayerNorm(32)
#         self.ln_target_enc=LayerNorm(12)
        ...
        self.time_embeddings = nn.Embedding(config.max_position_embeddings, 32)
        self.click_times_embeddings = nn.Embedding(32, 32)
        self.projection=nn.Linear(config.hidden_size*4,config.hidden_size)
        self.decoder_gender=nn.Sequential(nn.Linear(config.hidden_size*4+31,2))
        self.decoder_age=nn.Sequential(nn.Linear(config.hidden_size*4+31,10))
    ...
    def forward(self,times,click_times,create_embeddings,ad_embeddings,product_id_embeddings,                product_cat_embeddings,advertiser_embeddings,industry_embeddings,target_encodings,                product_cat_industry_embeddings,product_cat_advertisers,product_id_advertiser_embeddings,                product_id_industry_embeddings,product_id_product_cats,                advertiser_id_industrys,target_embed_info,                length,cal,tfidf,tfidf_sequence,one_hot_target,tr_sequence,graph_sequence,                age,gender,encoder_hidden_states=None,encoder_extended_attention_mask=None):
        norm_click_time=self.click_times_embeddings(click_times)
        X=torch.cat([self.ln_create(create_embeddings),self.ln_ad(ad_embeddings),self.ln_one_hot_target(one_hot_target),                     self.ln_advertiser(advertiser_embeddings),                     self.ln_product_id_advertiser(product_id_advertiser_embeddings),                     self.ln_product_cat_industry(product_cat_industry_embeddings),                     self.ln_tfidf_sequence(tfidf_sequence),                     self.ln_tr_sequence(tr_sequence)],dim=-1)

        #make attention mask
        attention_mask=self.make_mask(X,length)

#         attention_mask*=click_times.float()

        embedding_output=self.P(X,times)

#         assert (self.P.P==self.P.position_embeddings.weight.data.cpu()).all()
        #adjust attention mask
        if attention_mask.dim() == 3:
            extended_attention_mask = attention_mask[:, None, :, :]
        elif attention_mask.dim() == 2:
        ...
        outputs=self.Encoder(  embedding_output,
                ...
                encoder_attention_mask=encoder_extended_attention_mask,click_times=click_times)
        X=outputs[0]
#         X=self.projection(torch.cat([embedding_output,X,X-embedding_output,X*embedding_output],dim=-1))

        begin_hiddens,end_hiddens=[],[]
        for i in range(1):
            total_length=X.shape[1]
            new_X=pack_padded_sequence(X,length,batch_first=True,enforce_sorted=False)
            hidden,_=self.rnns[i](new_X)
            hidden,_=pad_packed_sequence(hidden,total_length=total_length,batch_first=True)
            hidden_avg = torch.sum(hidden * attention_mask.unsqueeze(1).transpose(2, 1), dim=1)                                         / torch.sum(attention_mask, dim=1, keepdim=True)
...
#             idx=length.unsqueeze(-1).repeat(1,1,self.config.hidden_size*2).permute(1,0,2)-self.one.long()
#             end_hidden=torch.gather(hidden,dim=1,index=idx).squeeze(1)
#             begin_hidden=hidden[:,0]
#             begin_hiddens.append(begin_hidden)
#             end_hiddens.append(end_hidden)
        ...
        #self.ln_cal(cal),self.ln_tfidf(tfidf)
        output_age=self.decoder_age(torch.cat([hidden,self.ln_cal(cal)],dim=-1))
        output_gender=self.decoder_gender(torch.cat([hidden,self.ln_cal(cal)],dim=-1))
        loss_age=LabelSmoothingCrossEntropy(0.1,weights=age_nums.to(self.device))
        loss_gender=LabelSmoothingCrossEntropy(0.1,weights=gender_nums.to(self.device))
        l=0.5*loss_age(output_age,age.long())+0.5*loss_gender(output_gender,gender.long())
        return l,output_age,output_gender
...
pattern='train'
config=BertConfig(**json.load(open("./transformer_full_config.json","r")),              output_hidden_states=True, output_attentions=True)
...
for fold in tqdm(range(5)):  
    ...
    batch_size = 256
    adam_epsilon=1e-8
    model=Ad_Transformer(config,device)
#     output_dir="../../model/fjw/model_wv200_"+str(fold+1)+"/"
    output_dir="../../model/fjw/model_transformer_wv50_full_shuffle_length100_reverse_"+str(fold+1)+"/"
    card_list=[0,2,3]
...
#         model.load_state_dict(torch.load(os.path.join(output_dir,"checkpoint-23048","model.pt")))
        ...
        model=model.to(args.device)
        results,age_preds,age_out_label_ids,gender_preds,gender_out_label_ids=        evaluate(args,temp_dataset,model,4,cross=True)
...
#     output_dir="../../model/fjw/model_wv200_"+str(fold+1)+"/"
    ...
    model.load_state_dict(torch.load(os.path.join(output_dir,"checkpoint-93750","model.pt")))
    model=model.to(args.device)
    results,age_preds,age_out_label_ids,gender_preds,gender_out_label_ids=    evaluate(args,test_dataset,model,4,cross=True)
    age_preds_list.append(age_preds)
    gender_preds_list.append(gender_preds)
