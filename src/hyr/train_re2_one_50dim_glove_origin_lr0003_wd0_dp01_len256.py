...
np.save('%s/id_embedding_wv_orgin.npy' % my_var_dir, id_embedding_wv_orgin)
...
id_embedding_feq = np.load('%s/id_embedding_wv_orgin.npy' % my_var_dir)
...
#     device=torch.device("cpu"),
    ...
    n_fold = 5,
    save_path = '%s/model_one_50dim_glove_origin_lr0003_wd0_dp01_len256/' % my_var_dir,
...
class RE2(nn.Module):
    def __init__(self, args):
        ...
        self.ln_50s = nn.ModuleList([LayerNorm(50) for _ in range(6)])
    ...
    def forward(self,
        ...
        a_wv = torch.cat([
            self.ln_50s[0](creative_id),
            self.ln_50s[1](ad_id),
            self.ln_50s[2](product_id),
            self.ln_50s[3](product_category),
            self.ln_50s[4](advertiser_id),
            self.ln_50s[5](industry),
