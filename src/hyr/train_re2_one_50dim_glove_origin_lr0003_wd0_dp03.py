...
# id_embeddings = np.load('var/id_embedding.npy')
...
# id_embedding_wv_orgin = np.concatenate([creative_id, ad_id, product_id, product_category, advertiser_id, industry], axis=1)
# np.save('var/id_embedding_glove_orgin.npy', id_embedding_wv_orgin)
...
# np.save('id_embedding_glove_short.npy', id_embedding_glove_short)
...
id_embedding_feq = np.load('%s/id_embedding_glove100origin.npy' % my_var_dir)
...
#     device=torch.device("cpu"),
    ...
    n_fold = 5,
    save_path = '%s/model_one_50dim_origin100glove_lr0003_wd0_dp03/' % my_var_dir,
...
def collate_fn(samples):
    ...
    for i, user in enumerate(user_ids):
        ...
        x_dict['creative_id'][i][:len_data] = id_embeddings[:, :100]
        x_dict['ad_id'][i][:len_data] = id_embeddings[:, 100:200]
        x_dict['product_id'][i][:len_data] = id_embeddings[:, 200:300]
        x_dict['product_category'][i][:len_data] = id_embeddings[:, 300:400]
        x_dict['advertiser_id'][i][:len_data] = id_embeddings[:, 400:500]
        x_dict['industry'][i][:len_data] = id_embeddings[:, 500:600]
...
class RE2(nn.Module):
    def __init__(self, args):
        ...
        self.ln_100s = nn.ModuleList([LayerNorm(100) for _ in range(6)])
...
#         self.ln_tfidf_stack = LayerNorm(55)
    ...
    def forward(self,
        ...
        product_category = product_category.to(args.device)
        advertiser_id = advertiser_id.to(args.device) 
        industry = industry.to(args.device)
        ...
        a_wv = torch.cat([
            self.ln_100s[0](creative_id),
            self.ln_100s[1](ad_id),
            self.ln_100s[2](product_id),
            self.ln_100s[3](product_category),
            self.ln_100s[4](advertiser_id),
            self.ln_100s[5](industry),
...
#         cat = torch.cat([self.ln_hidden(hidden), self.ln_tfidf_stack(x_flatten)], dim = -1)
        output_age = self.decoder_age(hidden)
        output_gender = self.decoder_gender(hidden)
