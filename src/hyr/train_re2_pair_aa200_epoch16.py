...
id_embedding_feq = np.load('%s/id_embedding_aa200_epoch16.npy'%my_var_dir)
...
#     device=torch.device("cpu"),
    ...
    n_fold = 5,
    save_path =  '%s/model_pair_aa200_wv_epoch16/' % my_var_dir,
    shuffle_flod = False,
...
def collate_fn(samples):
    ...
    for i, user in enumerate(user_ids):
        ...
        x_dict['ad_id'][i][:len_data] = id_embeddings[:, :200]
        x_dict['advertiser_id'][i][:len_data] = id_embeddings[:, 200:400]
...
class RE2(nn.Module):
    def __init__(self, args):
        ...
        self.ln_200s = nn.ModuleList([LayerNorm(200) for _ in range(2)])
...
score_gender_val, score_age_val, hidden_val = nn_cross_validation_multi_task(sub_train_user, sub_train_gender, sub_train_age,                                                                    sub_test_user,                                                                     RE2, {'args' : re2_args}, train_multi_task, True)
